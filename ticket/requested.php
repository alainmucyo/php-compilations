<?php 
  ...
  if (isset($_GET['id'])) {
  	$id=$_GET['id'];
  	$update=$conn->query("UPDATE ticket SET confirmed='1' WHERE id='$id'") or die("Ohhh Shit");
  }
  ...
  $sql=$conn->query("SELECT * FROM ticket WHERE confirmed='0' LIMIT $this_page_result,$result_per_page");
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav">
        ...
        <li class="requested active" style="display: none;"><a href="requested.php">Requested<span class="badge"><?=$ticket->num_rows ?></span></a></li>
...
<div class="col-md-10">
  <div class="w3-card-2">
    <header class="w3-container text-center bg-green"><h3>Requested Tickets</h3></header>
   ...
   <?php 
    while ($row=$sql->fetch_assoc()) {
     ?>
      ...
      <td><?=$row['place'] ?></td>
      <td><?=$row['price'] ?></td>
      <td><a href="requested.php?id=<?=$row['id'] ?>" class="w3-btn bg-blue">Accept</a></td>
      <td><a href="delete.php?action=delete&id=<?=$row['id'] ?> &table=ticket&header=requested" class="w3-btn bg-red">Delete</a></td>
      <td><?=$row['time'] ?></td>
      <td><?=$row['date'] ?></td>
       </tr>
      <?php }  ?>
    ...
    <div class="text-center">
    	<ul class="w3-pagination w3-border w3-tiny">
    		...
    		<li><a href="requested.php?page=<?=$page ?>" class="w3-border-right"><?=$page ?></a></li>
    		<?php } ?>
    	</ul>
    </div>
  </div>
