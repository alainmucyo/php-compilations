...
if (!isset($_SESSION['user'])) {
 ...
 include 'conn.php';
	$com=$conn->query("SELECT * FROM com");
	$dom=$conn->query("SELECT * FROM dom");
	$surv=$conn->query("SELECT * FROM surv");
	$sol=$conn->query("SELECT * FROM sol");
	$pro=$conn->query("SELECT * FROM pro");
	$maso=$conn->query("SELECT * FROM maso");
 ?>
<!DOCTYPE html>
<html>
<head>
...
<body>
	...
	<div class="container">
		<div class="w3-card-2 card">
			<header class="w3-container bg-blue text-center"><h1>Level Four</h1></header>
